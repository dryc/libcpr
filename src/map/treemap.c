...
static int
treemap_init(treemap_t* const restrict map, va_list args) {
  (void)map, (void)args;
  return FAILURE(ENOTSUP); // TODO
}

static int
treemap_dispose(treemap_t* const restrict map) {
  (void)map;
  return FAILURE(ENOTSUP); // TODO
}

static int
treemap_clear(treemap_t* const restrict map) {
  (void)map;
  return FAILURE(ENOTSUP); // TODO
}
...
public const class_t treemap_class = {
  .name    = "treemap",
  .super   = &map_class,
  .vtable  = &treemap_vtable.base,
};

public const map_vtable_t treemap_vtable = {
